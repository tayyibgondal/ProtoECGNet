...
from settings import base_architecture, experiment_run, img_size, log_dir, prototype_shape, num_prototypes_for_each_class, bootstraping
...
# ===================================================================================
if bootstraping:
    def _train_or_test(model, dataloader, optimizer=None, class_specific=True, use_l1_mask=True, coefs=None, log=log_to_file_and_console):
        is_train = optimizer is not None
        start = time.time()
        n_examples = 0
        n_correct = 0
        ...
        # Bootstrap sampling for AUROC calculation
        n_bootstrap = 1000
        rng_seed = 42
        ...
        for i in range(n_bootstrap):
            indices = resample(np.arange(len(all_labels_np)), replace=True)
            if len(np.unique(all_labels_np[indices])) < 2:
                continue
        ...
        ci_95_lower = sorted_scores[int(0.025 * len(sorted_scores))]
        ci_95_upper = sorted_scores[int(0.975 * len(sorted_scores))]
        
        ci_98_lower = sorted_scores[int(0.01 * len(sorted_scores))]
        ci_98_upper = sorted_scores[int(0.99 * len(sorted_scores))]
        
        ci_99_lower = sorted_scores[int(0.005 * len(sorted_scores))]
        ci_99_upper = sorted_scores[int(0.995 * len(sorted_scores))]
    
        log(f'\tAUROC: {np.mean(bootstrapped_auroc):.4f}')
        log(f'\tAUROC 95% CI: [{ci_95_lower:.4f} - {ci_95_upper:.4f}]')
        log(f'\tAUROC 98% CI: [{ci_98_lower:.4f} - {ci_98_upper:.4f}]')
        log(f'\tAUROC 99% CI: [{ci_99_lower:.4f} - {ci_99_upper:.4f}]')
    
        return (np.mean(bootstrapped_auroc), 
    ...
    def train(model, dataloader, optimizer, class_specific=False, coefs=None, log=print):
        ...
        log('\ttrain')
        model.train()
        auroc, ci_95_lower, ci_95_upper, ci_98_lower, ci_98_upper, ci_99_lower, ci_99_upper, ce, cc, sc, p_pair_dist = _train_or_test(
            model=model,
        ...
        wandb.log({
            "Train AUROC": auroc,
            "Train AUROC 95% CI Lower": ci_95_lower,
            "Train AUROC 95% CI Upper": ci_95_upper,
            "Train AUROC 98% CI Lower": ci_98_lower,
            "Train AUROC 98% CI Upper": ci_98_upper,
            "Train AUROC 99% CI Lower": ci_99_lower,
            "Train AUROC 99% CI Upper": ci_99_upper,
            "Train Cross Entropy": ce,
            "Train Cluster Cost": cc,
            "Train Separation Cost": sc if class_specific else 0,
            "Train P Dist Pair": p_pair_dist
    ...
    def test(model, dataloader, class_specific=False, log=print):
        ...
        wandb.log({
            "Test AUROC": auroc,
            "Test AUROC 95% CI Lower": ci_95_lower,
            "Test AUROC 95% CI Upper": ci_95_upper,
            "Test AUROC 98% CI Lower": ci_98_lower,
            "Test AUROC 98% CI Upper": ci_98_upper,
            "Test AUROC 99% CI Lower": ci_99_lower,
            "Test AUROC 99% CI Upper": ci_99_upper,
            "Test Cross Entropy": ce,
            "Test Cluster Cost": cc,
            "Test Separation Cost": sc if class_specific else 0,
            "Test P Dist Pair": p_pair_dist
