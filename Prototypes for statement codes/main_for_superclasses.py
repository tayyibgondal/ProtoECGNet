...
for epoch in range(num_train_epochs):
    ...
    save.save_model_w_condition(model=ppnet, model_dir=model_dir, model_name=str(epoch) + 'nopush', auroc=auroc, target_auroc=target_auroc, log=log)
    ...
    if epoch >= push_start and epoch in push_epochs:
        ...
        auroc = tnt.test(model=ppnet_multi, dataloader=test_loader,  
                        class_specific=class_specific, log=log)
        save.save_model_w_condition(model=ppnet, model_dir=model_dir, model_name=str(epoch) + 'push', auroc=auroc, target_auroc=target_auroc, log=log)
        ...
        if prototype_activation_function != 'linear':
            ...
            for i in range(20):
                ...
                auroc = tnt.test(model=ppnet_multi, dataloader=test_loader, 
                                class_specific=class_specific, log=log)
                save.save_model_w_condition(model=ppnet, model_dir=model_dir, model_name=str(epoch) + '_' + str(i) + 'push', auroc=auroc, target_auroc=target_auroc, log=log)
