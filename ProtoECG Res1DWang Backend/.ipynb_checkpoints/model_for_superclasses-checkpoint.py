...
class PPNet(nn.Module):
    ...
    def __init__(self, features, img_size, prototype_shape,
        ...
        if freq == 100:
            first_add_on_filter = 126
            second_add_on_filter = 125
        else:
            first_add_on_filter = 626
            second_add_on_filter = 625
        ...
        if add_on_layers_type == 'bottleneck':
            ...
            self.add_on_layers = nn.Sequential(
                nn.Conv1d(in_channels=first_add_on_layer_in_channels, out_channels=self.prototype_shape[1], kernel_size=first_add_on_filter),
                nn.ReLU(),
                nn.Conv1d(in_channels=self.prototype_shape[1], out_channels=self.prototype_shape[1], kernel_size=second_add_on_filter),
                nn.Sigmoid()
                )
...
def construct_PPNet(img_size=224,
                    prototype_shape=(2000, 512, 1, 1), num_classes=200,
                    prototype_activation_function='log',
                    add_on_layers_type='bottleneck'):
    features = resnet1d_wang(input_channels=12)
