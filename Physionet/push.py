...
def update_prototypes_on_batch(search_batch_input,
    ...
    if class_specific:
        ...
        # ------------------------
        # Create a class to images in current batch lookup.
        class_to_img_index_dict = {key: [] for key in range(num_classes)}
        # Get indices of the classes which are activated for each example
        non_zero_indices = [list(row.nonzero(as_tuple=True)[0].tolist()) for row in search_y]
        for example_number, row in enumerate(non_zero_indices):
            for activated_class in row:
                class_to_img_index_dict[activated_class] += [example_number]
        # ------------------------
